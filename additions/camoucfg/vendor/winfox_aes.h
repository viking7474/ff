...
#define WINFOX_AES_BLOCKLEN 16
#define WINFOX_AES256_KEYLEN 32
#define WINFOX_AES256_KEYEXP_SIZE 240
...
static inline uint8_t WINFOX_AES_xtime(uint8_t x) { return (uint8_t)((x << 1) ^ (((x >> 7) & 1U) * 0x1bU)); }
static inline uint8_t WINFOX_AES_Multiply(uint8_t x, uint8_t y) {
...
static inline void WINFOX_AES_KeyExpansion(uint8_t* RoundKey, const uint8_t* Key) {
...
static inline void WINFOX_AES_AddRoundKey(uint8_t round, WinfoxAES_state_t* state, const uint8_t* RoundKey) {
  uint8_t i, j; for (i = 0; i < 4; ++i) for (j = 0; j < 4; ++j) (*state)[i][j] ^= RoundKey[(round * WINFOX_AES_NB * 4) + (i * WINFOX_AES_NB) + j];
}
static inline void WINFOX_AES_InvSubBytes(WinfoxAES_state_t* state) {
  uint8_t i, j; for (i = 0; i < 4; ++i) for (j = 0; j < 4; ++j) (*state)[j][i] = WINFOX_AES_rsbox[(*state)[j][i]];
}
static inline void WINFOX_AES_InvShiftRows(WinfoxAES_state_t* state) {
  uint8_t temp;
...
static inline void WINFOX_AES_InvMixColumns(WinfoxAES_state_t* state) {
...
static inline void WINFOX_AES_InvCipher(WinfoxAES_state_t* state, const uint8_t* RoundKey) {
...
static inline void WINFOX_AES_XorWithIv(uint8_t* buf, const uint8_t* Iv) { uint8_t i; for (i = 0; i < WINFOX_AES_BLOCKLEN; ++i) buf[i] ^= Iv[i]; }

static inline void WinfoxAES_init_ctx_iv(struct WinfoxAES_ctx* ctx, const uint8_t* key, const uint8_t* iv) { WINFOX_AES_KeyExpansion(ctx->RoundKey, key); memcpy(ctx->Iv, iv, WINFOX_AES_BLOCKLEN); }
static inline void WinfoxAES_ctx_set_iv(struct WinfoxAES_ctx* ctx, const uint8_t* iv) { memcpy(ctx->Iv, iv, WINFOX_AES_BLOCKLEN); }
static inline void WinfoxAES_CBC_decrypt_buffer(struct WinfoxAES_ctx* ctx, uint8_t* buf, uint32_t length) {
