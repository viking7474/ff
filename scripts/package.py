...
def add_includes_to_package(package_file, includes, fonts, new_file, target):
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if target == 'macos':
            # Move Winfox/Winfox.app -> Winfox.app
            nightly_dir = os.path.join(temp_dir, 'Winfox')
            shutil.move(
                os.path.join(nightly_dir, 'Winfox.app'), os.path.join(temp_dir, 'Winfox.app')
            )
            # Remove old app dir and all content in it
            shutil.rmtree(nightly_dir)
        else:
            # Move contents out of winfox folder if it exists
            old_winfox_dir = os.path.join(temp_dir, 'winfox')
            winfox_dir = os.path.join(temp_dir, 'winfox-folder')
            if os.path.exists(old_winfox_dir):
                os.rename(old_winfox_dir, winfox_dir)
                for item in os.listdir(winfox_dir):
                    shutil.move(os.path.join(winfox_dir, item), temp_dir)
                os.rmdir(winfox_dir)

        # Create target_dir
        if target == 'macos':
            target_dir = os.path.join(temp_dir, 'Winfox.app', 'Contents', 'Resources')
        else:
            target_dir = temp_dir
...
def main():
    ...
    with temp_cd(src_dir):
        ...
        search_path = os.path.abspath(
            f'obj-{moz_target}/dist/winfox-{args.version}-{args.release}.*.{file_ext}'
        )
    ...
    package_pattern = f'winfox-{args.version}-{args.release}.en-US.*.{file_ext}'
    ...
    new_name = f'winfox-{args.version}-{args.release}-{args.os[:3]}.{args.arch}.zip'
